...
export default function VendorWithdrawalsPage() {
  ...
  const fetchWithdrawals = async () => {
    try {
      const response = await fetch('http://81.162.55.70:8000/api/vendor/withdrawals', {
        headers: {
  ...
  const createWithdrawal = async () => {
    try {
      const response = await fetch('http://81.162.55.70:8000/api/vendor/withdrawals/create', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
